...
function swapHTML(container, html) {
  ...
  // apiErrText + ago live in helpers.js (pure, unit-tested; loaded before this file).
  ...
  function wireComments(root) {
    ...
    // The add-comment button is disabled while the composer is empty
    // (whitespace-only counts as empty) — nothing to submit, so no live button.
    function syncSubmit() { submitBtn.disabled = !(input.value || '').trim(); }
    function submit() {
    ...
    syncSubmit(); // start disabled: the composer opens empty
    input.addEventListener('keydown', function(e){
      if (e.key === 'Escape') { e.preventDefault(); e.stopPropagation(); input.value = ''; input.blur(); sayStatus(status, ''); syncSubmit(); }
