...
Card type schemas have [their own page](DEVELOPER-REFERENCE-SCHEMA-AUTHORING.md).
...
the MCP `breaches` tool ([using Cards](OPERATIONS.md#history-events-breaches)).
...
[built vs proposed](INTEGRATOR-REFERENCE.md) tracks it.

## Related

- [Card definitions](DEVELOPER-REFERENCE-SCHEMA-AUTHORING.md) — field schemas
  and versioning.
- [Card type examples](DEVELOPER-REFERENCE-TYPES-EXAMPLES.md) — complete
  worked schemas.
- [Using Cards](OPERATIONS.md) — operating on cards from CLI, HTTP, and MCP.
- [Query DSL](../spec/SPEC-QUERY-DSL.md) — the filter grammar used by
