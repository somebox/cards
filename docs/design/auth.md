...
"Security / Actor" subsection promotes into `docs/spec/api-surface.md`
...
- We do **not** rewrite history. Event log integrity beats retroactive
  correction.
- A single `design-notes.md` bullet dates the flag day — "honest attribution
...
- `docs/spec/api-surface.md` — receives a "Security / Actor"
  subsection when this doc's reference implementation lands.

## 13. Promotion path

1. This document lands as `status: proposed`. ✓ (2026-07-10; frozen — see
   `docs/design-notes.md` freeze entry. Changes only via impl PRs that discover
...
4. On first impl landing, a "Security / Actor" subsection is added to
   `docs/spec/api-surface.md`. This doc's status becomes `spec` and
